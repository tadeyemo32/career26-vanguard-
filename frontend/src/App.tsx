...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/keys`).then(r => r.json()).then((d: Record<string, any>) => {
      const bools: KeyStatus = {};
  ...
  return (
    <div className="flex h-full bg-[#09090f] text-[#d8dce8]">
      ...
      <motion.aside initial={{ x: -200 }} animate={{ x: 0 }} transition={{ type: 'spring', stiffness: 100, damping: 20 }}
        ...
        <div className="px-5 py-4 border-t border-[#1e2235] flex items-center justify-between">
          <span className="text-[11px] text-[#6b7494]">v1.5.0</span>
